...
class Settings(BaseSettings):
    ...
    min_sample_size: int = Field(
        ...
        description="Minimum observations required before computing performance statistics.",
    )
    p95_outliers_threshold_ms: int = Field(
        default=10_000,
        alias="P95_OUTLIERS_THRESHOLD",
        description=(
            "Maximum response time, in milliseconds, allowed before treating a value as an "
            "outlier when assessing the import P95."
        ),
    )
    import_accuracy_threshold: float = Field(
        ...
        description=(
            "Minimum acceptable accuracy (0-1) when validating imported performance data."
        ),
    )
    alpha: float = Field(
        default=0.05,
        alias="ALPHA",
        description=(
            "Significance level (0-1) applied to statistical tests for import performance"
            " summaries."
        ),
    )
